...
TEST_KAFKA_BROKER = '192.168.99.100:9092'
TX_GROUP = b'tx_group'
...
class KafkaBlockchain(object):
    ...
    def initialize_chain(self):
        if self.find_highest_offset(BLOCKCHAIN_TOPIC) == 0:# this is to initialize blockchain
    ...
    def read_and_validate_chain(self, offset=OffsetType.EARLIEST):
        ...
        for message in consumer:
            if message is not None:#added is not None
    ...
    def read_transactions(self, partition):
        print("docWaiting for the transactions on partition %s" % partition)
        tx_count = 0
    ...
    def mine(self):#blockchain related
        ...
        latest_offset = self.find_highest_offset(BLOCKCHAIN_TOPIC)
        if self.find_earliest_offset(BLOCKCHAIN_TOPIC) > self.last_offset:
            print('New blocks found, appending to our chain')
            self.read_and_validate_chain(latest_offset) #write and validate maybe?
    ...
    def find_highest_offset(self,topic_name):
        #latest = self.get_topic(BLOCKCHAIN_TOPIC).latest_available_offsets()
        latest = self.get_topic(topic_name).latest_available_offsets()
        # TODO: We are only using topic partition 0 at this point
        return latest[0].offset[0]

    def find_earliest_offset(self,topic_name):
        
        earliest = self.get_topic(topic_name).earliest_available_offsets()
        # TODO: We are only using topic partition 0 at this point
        return earliest[0].offset[0]
